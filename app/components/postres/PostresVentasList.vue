...
<template>
  <div class="mt-6 flex-1 min-h-0">
    ...
    <div v-else-if="!ventas.length" class="flex h-full min-h-[200px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 py-8 text-center px-4">
    ...
    <ul v-else class="space-y-3 overflow-y-auto max-h-[30vh] md:max-h-[220px] pr-1 scrollbar-thin scrollbar-thumb-slate-200">
      <li v-for="venta in ventas" :key="venta._id" class="group flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50/50 p-3 transition-colors hover:bg-slate-50">
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-slate-100/80">
            <UIcon name="lucide:shopping-bag" class="h-5 w-5 text-slate-500" />
          </div>
          <div>
            <p class="text-sm font-bold text-slate-900">{{ postres.find(p => p._id === venta.postreId)?.name }}</p>
            <p class="text-xs font-semibold text-slate-500">{{ formatDate(venta.date) }}</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
