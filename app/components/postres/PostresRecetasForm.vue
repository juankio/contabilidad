  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="space-y-2">
        ...
        <UInput v-model="name" placeholder="Ej: Torta 3 leches" size="lg" class="font-medium" icon="lucide:cake" />
      </div>
      <div class="space-y-2">
        <label class="text-xs font-semibold uppercase tracking-wider text-slate-500">Precio de Venta</label>
        <UInput v-model="price" placeholder="0" inputmode="numeric" size="lg" class="font-medium" :ui="{ icon: { leading: { pointer: '' } } }">
    ...
    <div class="space-y-4 rounded-xl border border-slate-100 bg-slate-50 p-4">
      <div class="flex items-center justify-between">
         ...
         <UBadge color="neutral" variant="subtle">{{ localReceta.length }} insumos</UBadge>
      </div>

      <div class="flex flex-col sm:flex-row gap-2">
        <USelect v-model="selectedInsumoId" :options="insumos.map(i => ({ label: i.name, value: i._id }))" size="lg" class="font-medium flex-1" icon="lucide:package" placeholder="Seleccionar Insumo" />
        <div class="flex gap-2 sm:w-1/2">
          <UInput v-model="yields" placeholder="Cant." inputmode="numeric" size="lg" class="w-full font-medium" icon="lucide:scale" />
          <UButton color="neutral" variant="outline" icon="lucide:plus" size="lg" @click="addLocalInsumo" />
        </div>
      </div>

      <!-- Lista Temporal de Insumos -->
      <ul v-if="localReceta.length > 0" class="space-y-2 mt-4 max-h-40 overflow-y-auto pr-1 scrollbar-thin">
        <li v-for="(item, idx) in localReceta" :key="idx" class="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-slate-100 shadow-sm">
          <div class="flex items-center gap-2">
            <UIcon name="lucide:flask-conical" class="h-4 w-4 text-slate-400" />
          ...
          <div class="flex items-center gap-3">
            ...
            <UButton color="error" variant="ghost" icon="lucide:x" size="xs" @click="removeLocalInsumo(idx)" />
          </div>
        </li>
      </ul>
    </div>
    ...
    <div class="pt-4 flex flex-col gap-2">
      <UButton color="primary" icon="lucide:check-circle" block size="lg" class="font-bold shadow-md" :loading="submitting" @click="submit">
