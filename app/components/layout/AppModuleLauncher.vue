...
<template>
  <UPopover v-model:open="open" :content="{ side: 'bottom', align: 'end', sideOffset: 8 }">
    ...
    >
      <UIcon name="lucide:layout-grid" class="size-5 shrink-0" />
      <span>Módulos</span>
    </button>
    ...
    <template #content>
      <div class="w-[420px] p-4 bg-white/95 backdrop-blur-2xl rounded-[2rem] shadow-xl border border-slate-100/80">
        <div class="mb-4 px-2">
          <h3 class="text-sm font-bold tracking-tight text-slate-900">Tus Módulos</h3>
          <p class="text-xs font-medium text-slate-500">Herramientas activas en este espacio.</p>
        </div>
        
        <div v-if="apps.length === 0" class="py-8 text-center flex flex-col items-center gap-2">
          <UIcon name="lucide:blocks" class="size-8 text-slate-200" />
          <p class="text-xs font-semibold text-slate-400">No hay módulos extra activados.</p>
        </div>
        ...
        <div v-else class="grid grid-cols-2 gap-2">
          ...
          >
            <div class="flex h-8 w-8 items-center justify-center rounded-xl bg-[var(--brand-50)] text-[var(--brand-600)] ring-1 ring-inset ring-[var(--brand-500)]/20 shadow-sm transition-transform group-hover:scale-110">
              <UIcon :name="String(app.icon)" class="size-4" />
            </div>
            <div class="mt-1">
        ...
        <div class="mt-3 border-t border-slate-100 pt-3 px-2">
          <NuxtLink
            ...
            @click="open = false"
          >
            <UIcon name="lucide:settings-2" class="size-4" />
            Activar más módulos
          </NuxtLink>
        </div>
      </div>
