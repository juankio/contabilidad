  ...
  <div class="h-full rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 transition-all hover:shadow-md flex flex-col min-w-0 w-full">
    <h3 class="text-sm font-bold tracking-tight text-slate-900 mb-6 flex items-center gap-2 shrink-0">
      <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--brand-50)] text-[var(--brand-600)] ring-1 ring-[var(--brand-500)]/20 shadow-sm">
        <UIcon name="lucide:pie-chart" class="h-4 w-4" />
      </div>
      Gastos por Categoría
    </h3>
    
    <div v-if="!props.categorias.length" class="flex-1 flex flex-col items-center justify-center text-slate-400 gap-3">
      <UIcon name="lucide:ghost" class="h-8 w-8 opacity-50" />
      <span class="text-sm font-medium">Sin gastos registrados.</span>
    </div>

    <div v-else class="flex flex-col items-center gap-8 flex-1 min-w-0 py-2">
      <!-- Doughnut SVG -->
      <div class="relative h-44 w-44 shrink-0 mx-auto">
        <svg viewBox="0 0 42 42" class="h-full w-full -rotate-90 transform drop-shadow-md">
          <circle
        ...
        >
          <div class="flex items-center justify-between text-sm min-w-0 w-full">
            <div class="flex items-center gap-2.5 min-w-0 flex-1 pr-2">
              <span class="h-2.5 w-2.5 shrink-0 rounded-full shadow-sm" :style="{ backgroundColor: cat.color }" />
          ...
          <div class="h-1.5 w-full overflow-hidden rounded-full bg-slate-100 flex">
            <div class="h-full transition-all duration-1000 ease-out rounded-full" :style="{ backgroundColor: cat.color, width: `${totalValue ? (cat.value / totalValue) * 100 : 0}%` }" />
          </div>
        </div>
      </div>
    </div>
