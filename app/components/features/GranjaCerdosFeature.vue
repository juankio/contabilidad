...
import type { LotePayload, ConcentradoPayload } from '../../composables/granja/useGranjaCerdos'

const { lotes, loading, fetchLotes, crearLote, comprarConcentrado, editarLote, eliminarLote } = useGranjaCerdos()
const toast = useToast()

const handleCrearLote = async (payload: LotePayload, onSuccess: () => void) => {
  try {
...
const handleComprarConcentrado = async (payload: ConcentradoPayload, onSuccess: () => void) => {
  try {
